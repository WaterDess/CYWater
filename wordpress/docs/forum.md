...
Nothing in this module is enabled on staging or production until the association
approves it. It ships inactive.
...
WordPress on Playground. 18 checks pass:
...
Not covered, and still required on staging:

1. Real PMPro membership state. Playground uses SQLite and cannot run PMPro's
   membership queries, so `pmpro_hasMembershipLevel` is stubbed in both the test
   and the preview. Every membership assertion above is therefore about the
   plugin's own logic, not about PMPro.
2. Real Postmark delivery of the endorsement and authorisation messages.
3. Browser review at desktop and 375px, including the PMPro order and account
   lists, whose bullet reset moved to `wordpress.css` (see below).
4. The moderation queue with a real Editor account.
