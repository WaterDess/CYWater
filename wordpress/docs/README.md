...
does not require a percentage of a long article to become visible.
...
baseline uses the CYWater `0.6.61` theme, CYWater Membership `0.9.14`, CYWater
...
details, fee and member-discount rules, external-payment and invoice guidance,
accommodation/shuttle details, and contacts. The former off-site reply-form
instruction is intentionally represented by the existing protected CYWater
...
PMPro remains authoritative for administrator-side membership, order,
subscription, billing-correction and refund operations. Theme `0.6.51` uses the
same support presentation without hiding active controls in CSS or editing
PMPro vendor files. Staging Account QA passed 92 assertions, and the production
...
Membership `0.9.8` centralizes public-member versus staff routing: registration,
...
linked to the Account membership section. Public Change and Cancel controls are
not offered. Membership-level changes and cancellations are handled through
`membership@cywater.org`, while same-level renewal and administrator-side PMPro
