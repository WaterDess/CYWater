...
| `plugins/cywater-best-paper` | Award-scoped cycle configuration, private applications and PDF files, committee assignments, independent reviews, advisory score summaries, explicit decisions and protected exports | Account/membership changes, automatic winner selection, News publication, award payments, certificate issuance or AGU-system uploads |
| `plugins/cywater-meeting-registration` | Reusable Event-scoped Annual Meeting registration, Event Tickets attendee snapshots, participant/staff protected-file review, accommodation intent, fee recommendation, staff CSV and structured ZIP export | Membership mutation, Stripe/PMPro payment, external Alipay confirmation, hotel booking or invoice issuance |
...
| `plugins/cywater-operations` | Composable operational roles, minimal role/workflow audit, paid-Event terms approval and registration-readiness gate, sole Logo review UI, and the dedicated Program Reviewer capability shared by Logo and Annual Meeting review workspaces | Logo/Meeting submission ownership, protected-file storage, Event editing through the reviewer capability, payment confirmation, PMPro membership/order administration, Stripe/Event Tickets transactions, event refunds, policy approval outside WordPress |
