...
final class CYWater_Forum_Content {
	...
	/**
	 * Prevent a member author from restoring any trashed Forum article.
	 *
	 * Authors may take down their own work, but a record in Trash is under the
	 * staff recovery boundary. This blocks both the ordinary admin action and a
	 * direct `wp_untrash_post()` call. The capability mapper independently
	 * closes REST and edit-screen updates while the record remains in Trash.
	 *
	 * An earlier filter may already have made a decision; never override it.
	 *
	 * @param bool|null $untrash         Whether to continue untrashing.
	 * @param WP_Post   $post            Post being restored.
	 * @param string    $previous_status Status before the post was trashed.
	 * @return bool|null
	 */
	public static function prevent_nonstaff_untrash( $untrash, $post, $previous_status ) {
		...
		if ( CYWater_Forum_Roles::is_staff( get_current_user_id() ) ) {
			return $untrash;
		}
		return false;
	}
	...
	/**
	 * Staff restore a Forum article to its previous status.
	 *
	 ...
	 * Retaining the former status is useful for staff recovery. Non-staff never
	 * reach this filter because `prevent_nonstaff_untrash()` rejects the action;
	 * returning the core status here is a defense-in-depth fallback.
	 *
	...
	/**
	 * Hide the dead Restore action from non-staff Trash screens.
	 *
	 * Authorization remains enforced by the capability and pre-untrash gates;
	 * this filter only avoids presenting an action that will always be denied.
	 *
	 * @param array<string, string> $actions Row actions.
	 * @param WP_Post               $post    Current row.
	 * @return array<string, string>
	 */
	public static function remove_nonstaff_untrash_action( $actions, $post ) {
		if (
			$post instanceof WP_Post
			&& self::POST_TYPE === $post->post_type
			&& 'trash' === $post->post_status
			&& ! CYWater_Forum_Roles::is_staff( get_current_user_id() )
		) {
