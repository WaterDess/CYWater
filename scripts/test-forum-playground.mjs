...
/**
 * Endorsement ships off. The endorsement sections below still have to exercise
 * the machinery, so the switch is flipped explicitly around them rather than
 * left to whatever the default happens to be.
 */
async function setEndorsement(on) {
  await php(`
$settings = array_merge( CYWater_Forum_Settings::defaults(), (array) get_option( 'cywater_forum_settings', array() ) );
$settings['endorsement_required'] = ${on ? "true" : "false"};
...
echo wp_json_encode( $out );`);
  ...
  assert.equal(registered.settings.endorsement_required, false, "Endorsement is off for launch");
  ...
  record("approved policy defaults load", "endorsement off, membership required, override on");

  /* ------------------------------------------------------------------
   * 2. Users and the publishing gate, blocker by blocker
   * ---------------------------------------------------------------- */
  await setEndorsement(true);
...
echo wp_json_encode( array( 'author_id' => $author_id, 'candidate_id' => $cand_id, 'stages' => $stages ) );`);
  ...
  /* ------------------------------------------------------------------
   * 2b. Open publishing: the launch policy.
   *
   * Endorsement is off, so an active, email-verified member publishes with no
   * endorsement at all. Board members are exempt from dues rather than from
   * membership: an administrator gives them a complimentary level with no
   * order, which is what the second case here stands in for.
   * ---------------------------------------------------------------- */
  await setEndorsement(false);
  const openPublishing = await json(`
$open = wp_insert_user( array( 'user_login' => 'paid.member', 'user_email' => 'paid@example.org', 'user_pass' => wp_generate_password(), 'display_name' => 'Paid Member', 'role' => 'subscriber' ) );
update_user_meta( $open, 'cyw_verified_email', 'paid@example.org' );

$before_paying = CYWater_Forum_Roles::publish_blockers( $open );
update_user_meta( $open, 'test_active_membership', 1 );
$after_paying = CYWater_Forum_Roles::publish_blockers( $open );

// A complimentary level is indistinguishable from a paid one to the gate,
// which is exactly how a Board member publishes without paying dues.
$board = wp_insert_user( array( 'user_login' => 'board.member', 'user_email' => 'board@example.org', 'user_pass' => wp_generate_password(), 'display_name' => 'Board Member', 'role' => 'subscriber' ) );
update_user_meta( $board, 'cyw_verified_email', 'board@example.org' );
update_user_meta( $board, 'test_active_membership', 1 );

$post_id = wp_insert_post( array(
    'post_type'   => 'cyw_forum_post',
    'post_status' => 'publish',
    'post_title'  => 'Published without any endorsement',
    'post_author' => $open,
) );

echo wp_json_encode( array(
    'before_paying'    => $before_paying,
    'after_paying'     => $after_paying,
    'cap_after_paying' => user_can( $open, 'publish_cyw_forum_posts' ),
    'board_blockers'   => CYWater_Forum_Roles::publish_blockers( $board ),
    'board_cap'        => user_can( $board, 'publish_cyw_forum_posts' ),
    'endorsements'     => count( CYWater_Forum_Endorsement::endorsements( $open ) ),
    'post_live'        => 'publish' === get_post_status( $post_id ),
    'post_id'          => $post_id,
) );`);

  assert.deepEqual(
    openPublishing.before_paying,
    ["membership_inactive"],
    "Without dues the only thing standing in the way is membership"
  );
  assert.deepEqual(openPublishing.after_paying, [], "A paid, verified member may publish with no endorsement");
  assert.equal(openPublishing.cap_after_paying, true);
  assert.equal(openPublishing.endorsements, 0, "No endorsement was involved");
  assert.deepEqual(openPublishing.board_blockers, [], "A complimentary level lets a Board member publish without dues");
  assert.equal(openPublishing.board_cap, true);
  assert.equal(openPublishing.post_live, true);
  record("open publishing: dues and a verified email are the whole gate", "no endorsement required");
  record("complimentary level lets a Board member publish without paying dues");

  await php(`wp_set_current_user( 1 ); wp_delete_post( ${openPublishing.post_id}, true ); echo 'cleaned';`);
  await setEndorsement(true);
